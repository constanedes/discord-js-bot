...
export default new Command({
    ...
    async run({ interaction }) {
        interaction.reply({
            ...
            components: [
                createRow(new ButtonBuilder({ customId: "ping-button", label: "ping", style: ButtonStyle.Success })),
            ],
        });
    },
    components: [
        new Component({
            ...
            async run(interaction) {
                ...
                interaction.update({
                    embeds: [new EmbedBuilder({ description, color: hexToRgb(settings.colors.theme.primary) })],
                });
            },
        }),
    ],
